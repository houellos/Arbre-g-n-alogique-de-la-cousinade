...
def select_working_folder():
    ...
    # Création des 4 dossiers s'ils n'existent pas
    folders = ["Portraits", "Photos", "Anecdotes", "Modifications"]
    for f in folders:
    ...
    return ged_files[0], folder_path, os.path.join(folder_path, "index.html")

GEDCOM_FILE, PROJECT_ROOT, OUTPUT_FILE = select_working_folder()
...
# --- 2. GESTION DES PHOTOS ET NOTES (NOUVELLE STRUCTURE) ---
def scan_media_for_individual(ind_id, root_path):
    main_photo = None
    gallery = []
    user_notes = [] 

    # 1. Portraits (Image principale uniquement)
    path_portraits = os.path.join(root_path, "Portraits")
    if os.path.exists(path_portraits):
        for ext in ['.jpg', '.jpeg', '.png', '.gif']:
            potential_file = f"{ind_id}{ext}"
            if os.path.exists(os.path.join(path_portraits, potential_file)):
                # On stocke le chemin relatif pour le HTML
                main_photo = f"Portraits/{potential_file}"
                break # On prend la première trouvée
    
    # 2. Photos (Galerie)
    path_photos = os.path.join(root_path, "Photos")
    if os.path.exists(path_photos):
        for filename in os.listdir(path_photos):
            if filename.startswith(f"{ind_id}_"): # ex: I719_vacances.jpg
                ext = os.path.splitext(filename)[1].lower()
                if ext in ['.jpg', '.jpeg', '.png', '.gif']:
                    gallery.append(f"Photos/{filename}")

    # 3. Anecdotes (Texte)
    path_anecdotes = os.path.join(root_path, "Anecdotes")
    if os.path.exists(path_anecdotes):
        for filename in os.listdir(path_anecdotes):
            if filename.startswith(f"{ind_id}_") and filename.lower().endswith('.txt'):
                try:
                    with open(os.path.join(path_anecdotes, filename), 'r', encoding='utf-8') as f:
                        user_notes.append(f.read())
                except: pass

    # 4. Modifications : ON IGNORE (comme demandé)

    return main_photo, gallery, user_notes

# --- 3. PARSING GEDCOM ---
def parse_gedcom(file_path, root_path):
    individuals = {}
    families = {}
    try:
    ...
    print("Association des médias (Portraits, Photos, Anecdotes)...")
    for iid, ind in individuals.items():
        main, gal, notes = scan_media_for_individual(iid, root_path)
        ind['main_photo'] = main 
        ind['gallery'] = gal     
        for n in notes:
...
<body>
    ...
    <script>
        ...
        function openModal(id) {{
            ...
            // On affiche le portrait (depuis dossier Portraits)
            const headerImg = p.main_photo ? `${{p.main_photo}}` : (p.sex==='F'?'{img_f}':'{img_h}');
            ...
            // Galerie photos (depuis dossier Photos)
            if (p.gallery && p.gallery.length > 0) {{
                ...
                p.gallery.forEach(photo => {{ 
                    // Extraction nom propre pour l'affichage
                    let filename = photo.split('/').pop(); // recup I719_machin.jpg
                    let displayTitle = filename.split('.')[0].replace(id + "_", "").replace(/_/g, " ");
                    
                    html += `
                    <div style="text-align:center;">
                        <a href="${{photo}}" target="_blank"><img src="${{photo}}" class="gallery-img"></a>
        ...
        function renderCard(id) {{
            ...
            if(!i) return "";
            // Photo principale (Portraits)
            const iconSrc = i.main_photo ? `${{i.main_photo}}` : (i.sex==='F'?'{img_f}':'{img_h}');
        ...
        function buildBranch(personId, depth) {{
            ...
            if(ind.fams.length > 0) {{
                ...
                if(fam) {{
                    ...
                    if(spouseId) {{
                        // ANNEE SEULE SUR L'ARBRE
                        let marrYear = fam.marr ? fam.marr.slice(-4) : "";
                        spouseHtml = `<div class="connector"><span class="marr-date">💍 ${{marrYear}}</span></div>${{renderCard(spouseId)}}`;
                    }}
...
indi, fami = parse_gedcom(GEDCOM_FILE, PROJECT_ROOT)
